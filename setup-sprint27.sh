#!/usr/bin/env bash
# classroom-planner — setup-sprint27.sh
#
# Sprint 27: SuperTeach dashboard (Ring 3, React)
# - Fristående panel: kunskapsöversikt per elev, luckor, trend
# - Lärarinmatning av evidens, AI-granskningskö, JSON export/import
# - localStorage-adapter med EGEN nyckel + feature-flagga + lazy-load
#
# Kör i projektroten:
#   bash setup-sprint27.sh
#   npm test
#
...
echo ""
echo -e "${BLUE}══════════════════════════════════════════${NC}"
echo -e "${BLUE}  Classroom Planner — Sprint 27: SuperTeach dashboard (UI)  ${NC}"
echo -e "${BLUE}══════════════════════════════════════════${NC}"
echo ""
...
if [ ! -f "packages/core/src/features/superteach/service.ts" ]; then
  echo "❌  Sprint 26 krävs. Kör setup-sprint26.sh först."
  exit 1
fi
ok "Sprint 26 hittad"

# Hitta webbpaketet (skapar annars en väntkatalog i roten).
WEB=""
for c in packages/web packages/webapp apps/web; do
  if [ -d "$c/src" ]; then WEB="$c"; break; fi
done
if [ -z "$WEB" ]; then
  WEB="superteach-ui-pending"
  echo "⚠️   Inget webbpaket hittades — filerna läggs i $WEB/."
  echo "     Flytta katalogen till <webbpaket>/src/features/superteach/ när du vill aktivera."
  DEST="$WEB"
else
  DEST="$WEB/src/features/superteach"
fi
mkdir -p "$DEST"
ok "Mål: $DEST"

log ""$DEST"/superteachCore.ts..."
cat > "$DEST"/superteachCore.ts << 'ST27_SHIM'
/**
 * Lokal återexport av SuperTeach-kärnan så att UI-filerna har EN importpunkt.
 * Byt raden nedan till '@planner/core' om webbpaketet använder det aliaset.
 */
export * from '../../../../core/src/features/superteach/index.js';
ST27_SHIM
ok ""$DEST"/superteachCore.ts"
log ""$DEST"/localStorageEvidenceStore.ts..."
cat > "$DEST"/localStorageEvidenceStore.ts << 'ST27_STORE'
/**
...
ST27_STORE
ok ""$DEST"/localStorageEvidenceStore.ts"
log ""$DEST"/SuperTeachPanel.tsx..."
cat > "$DEST"/SuperTeachPanel.tsx << 'ST27_PANEL'
/**
...
ST27_PANEL
ok ""$DEST"/SuperTeachPanel.tsx"

echo ""
ok "Sprint 27 klar."
echo ""
echo "Aktivering (EN rad + en flik i App.tsx — inget annat rörs):"
echo ""
echo "  const SuperTeachPanel = React.lazy(() => import('./features/superteach/SuperTeachPanel'));"
echo "  ..."
echo "  {isSuperTeachEnabled() && <SuperTeachPanel students={elever} subject=\"matematik\" />}"
echo ""
echo "Slå på flaggan i webbläsarkonsolen:"
echo "  localStorage.setItem('classroom-planner.superteach.enabled','true')"
